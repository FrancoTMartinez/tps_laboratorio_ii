	...
	public partial class FrmVoto : Form
	{
							...
							try
							{
								if (Validar.Edades(int.Parse(this.txtEdad.Text)))
								{
									if (this.cmbGenero.SelectedIndex >= 0)
									{
										if (voto != null)
										{
											Persona persona = new Persona(this.txtDNI.Text, this.txtNombre.Text, this.txtApellido.Text, Canal.GetIdByName(voto), int.Parse(this.txtEdad.Text), this.cmbGenero.SelectedItem.ToString());
											persona.AddToDB(persona);
											Console.Beep();
		...
		private void btnBuscarPorDni_Click(object sender, EventArgs e)
		{
			...
			if (frmModificar.ShowDialog() == DialogResult.OK)
			{
				...
				voto = Canal.GetNameById(persona.IdCanal);
						...
						try
						{
							if (Validar.Edades(int.Parse(this.txtEdad.Text)))
							{
								if (this.cmbGenero.SelectedIndex >= 0)
								{
									...
									personaAModificar.IdCanal = Canal.GetIdByName(voto);

									if (Validar.ValidateExistingDNI(personaAModificar)) {

										if (personaAModificar.UpdateFromDB(personaAModificar))
										{
											Console.Beep();
											this.DialogResult = DialogResult.OK;
										}
										...
										{
											this.lblErrorDni.ForeColor = Color.Red;
											this.lblErrorDni.Text = "Ocurrio un error.";
										}
									}
									else
									{
										this.lblErrorDni.ForeColor = Color.Red;
										this.lblErrorDni.Text = "El DNI ingresado ya se encuentra registrado.";
										this.lblGeneroError.Text = string.Empty;
										this.lblErrorEdad.Text = string.Empty;
										this.lblErrorApellido.Text = string.Empty;
										this.lblErrorNombre.Text = string.Empty;
										this.lblErrorDni.Text = string.Empty;
		...
		private void btnEliminar_Click(object sender, EventArgs e)
		{
			try
			{
				...
				if (personaAEliminar.DeleteFromDB(personaAEliminar.Id))
				{
